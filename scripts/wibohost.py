...
class WIBOHost(WIBOHostBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.sport = serial.Serial(*args, **kwargs)
    ...
    def _flush(self):
        ...
        """
        self.sport.read(self.sport.inWaiting())
    ...
    def target(self, nodeid, targ):
        """ Set memory target
            'E' : EEPROM
            'F' : Flash memory
            'X' : No memory, dry run
        """
        return self._sendcommand('target', hex(nodeid), targ)
    ...
    def shortaddr(self, short_addr):
        """ Set my SHORT_ADDR """
        return self._sendcommand('shortaddr', hex(short_addr))
...
class WIBONetwork(WIBOHost):
    ...
    def scan(self, scanrange=None):
        ...
        if scanrange != None: # Scan given range of addresses
            for a in scanrange:
                tmp = self.ping(a)
                if self.VERBOSE >= 2:
        ...
        else: # Scan unknown address range
            lst=[]
            self.reset()
            self.nodes = NodeList()
            retry=0
    ...
    def flashhex(self, nodeid, fname):
...
def process_command_line():
    ...
    if ret == False:
        ...
        wnwk.sport.close()
        wnwk.sport.setTimeout(1.0)
        wnwk.sport.setPort(PORT)
        wnwk.sport.setBaudrate(BAUDRATE)
        wnwk.sport.open()
            ...
            if o == "-u":
                ...
                for n in ADDRESSES:
                    ...
                    if tmp['code'] == 'OK' and tmp['data']['appname'] == "wibo":
                            ...
                            print "Status Table"
                            wnwk.checkcrc() # fetch status of each node in NWK...
                            print wnwk.nodes # ...and print it
                            print "EXIT", n
                    else:
